...
@else
    ...
    "
        ...
        :class="{
            'inline-flex w-full cursor-pointer items-center justify-between gap-2 rounded-md bg-primary/20 px-4 py-2 text-left text-sm text-primary': (selectedValue === '{{ $value }}' || selectedValue.includes('{{$value}}')) &&
                !{{ $disabled ? 'true' : 'false' }},
            'w-full cursor-pointer rounded-md px-4 py-2 text-left text-sm hover:bg-accent hover:text-accent-text': selectedValue !== '{{ $value }}' &&
                !{{ $disabled ? 'true' : 'false' }},
        ...
        {{ $slot }}
        <template x-if="selectedValue === '{{ $value }}' || selectedValue.includes('{{$value}}')">
