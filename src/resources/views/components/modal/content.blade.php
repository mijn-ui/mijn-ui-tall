...
@push('modals')
    <x-slot name="content">
        <div x-bind:class="open ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'"
            class="z-[9999] fixed inset-0 bg-black/50 flex items-center justify-center p-4"
            aria-hidden="true">
            ...
            <div x-show="open" x-transition @click.away="open = false" @keydown.escape.window="open = false"
                ...
                <div class="relative">
                    <button type="button" x-on:click="open = false" class="z-[9998] absolute top-0 right-0 text-lg text-muted-text hover:text-foreground"
                        aria-label="Close">
