@props([
    ...
    'codeOnly' => false, // When true, only show the code block (no preview tab, no Blade::render)
])
...
<div {{ $attributes->class([$base, $class]) }}>
    @if ($codeOnly)
        {{-- Code-only mode: no preview tab, no Blade::render --}}
        <div class="text-start relative" x-data="{ copied: false }">
            <button
                x-on:click="
